...
function handleSave(event) {
	if (typeof Storage !== 'undefined') {
		...
		const variablesXml = Blockly.Xml.variablesToDom(workspace.getAllVariables())
		localStorage.setItem('blocklyWorkspace', Blockly.Xml.domToText(blocksXml))
...
function handleRestore(event) {
	if (typeof Storage !== 'undefined') {
		...
		if (blocksText) {
			...
			const workspace = Blockly.getMainWorkspace()
			workspace.clear()
			Blockly.Xml.domToWorkspace(Blockly.Xml.textToDom(blocksText), workspace)

			Blockly.Xml.domToVariables(variablesXml, workspace)

			console.log('restored ', variablesText, blocksText)
		}
	}
}
